...
func main() {
	...
	//2  Fprint  Fprint系列函数会将内容输出到一个io.Writer接口类型的变量w中，我们通常用这个函数往文件中写入内容。
	fmt.Fprintln(os.Stdout, "像标准输出写入内容")
	fileObj, err := os.OpenFile("fmt/printf/test.txt", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
	...
	//格式化占位符
	// 通用占位符 *printf系列函数都支持format格式化参数，在这里我们按照占位符将被替换的变量类型划分，方便查询和记忆。
	// %v 值的默认格式表示
